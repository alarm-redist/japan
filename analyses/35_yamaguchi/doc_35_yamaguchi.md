# 2022 Redistricting for the House of Representatives Electoral Districts in Yamaguchi Prefecture
...
* We add edges between islands and municipalities to which the islands are connected via ferry routes. In the case of Yamaguchi, we add an edge between 柳井市 (Yanai-shi) and 周防大島町 (Suooshima-cho).

## Simulation Notes

* We sample 20,000 districting plans across 4 independent runs of the Sequential Monte Carlo (SMC) algorithm.
* We do not allow municipalities to be split except for those that were split in the 2017 or 2022 map: 山口市 (Yamaguchi-shi) and 周南市 (Shunan-shi). We allow these municipalities to be split along the boundaries of pre-merger municipalities. 
* In addition, we do not allow counties (*gun*) to be split except for those that were split in the 2017 map. In the case of Yamaguchi, we do not allow any counties (*gun*) to be split.
