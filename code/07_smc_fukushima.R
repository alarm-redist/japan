...
for(i in 0:nsplit){
  ...
  #merge 伊達市 and 0730
  pref_n <- avoid_enclave(pref_n, c(7213, 7300))
  pref_n <- avoid_enclave(pref_n, c(7560, 7209))
  pref_n <- avoid_enclave(pref_n, c(7211, 7520))
  #------------- set up map ----------------
...
fukushima_7_optimalmap <- redist::redist.plot.map(shp = fukushima_7_0, plan = fukushima_7_smc_plans_0[, which(fukushima_7_smc_weight_0$max_to_min == min(fukushima_7_smc_weight_0$max_to_min))[1]], boundaries = FALSE, title = "Fukushima Optimal Plan (0-split)") +
  scale_fill_manual(values= c("1" = "blue", "2" = "red", "3" = "yellow",
                              "4" = "green")) + ggplot2::labs(caption = paste("Max-min Ratio: ", round(min(fukushima_7_smc_weight_0$max_to_min), 3), sep = ""), hjust = 0.5)
...
ggsave(filename = paste("plots/",
                        ...
                        "_optimal.png",
                        sep = ""),
       plot = fukushima_7_optimalmap)

status_quo <- status_quo_match(fukushima_7_0)

fukushima_orig_weight <- simulation_weight_disparity_table(redist::redist_plans(plans = matrix(status_quo$ku, ncol = 1), map = fukushima_7_map_0, algorithm = "smc"))

unique_weights <- fukushima_7_smc_weight_0
unique_weights$n <- 1:nsims
unique_weights <- unique_weights %>%
  dplyr::group_by(max_to_min, Gini, LH, HH) %>% summarize(n = first(n))

fukushima_overlap_0 <- vector(length = nrow(unique_weights))

for (i in 1:length(fukushima_overlap_0)){
  fukushima_overlap_0[i] <- redist::redist.prec.pop.overlap(status_quo$ku, fukushima_7_smc_plans_0[, unique_weights$n[i]], fukushima_7_0$pop,
                                                           weighting = "s", index_only = TRUE)
}

improved_plans <- as.data.frame(
  cbind(unique_weights %>% dplyr::filter(max_to_min < fukushima_orig_weight$max_to_min),

        c(fukushima_overlap_0[which(unique_weights$max_to_min < fukushima_orig_weight$max_to_min)]
        ),

        as.character(rep("0", length(which(unique_weights$max_to_min < fukushima_orig_weight$max_to_min))))
  ))

names(improved_plans) <- c(names(unique_weights), "Dissimilarity", "Splits")


fukushima_marginal <- ggplot(improved_plans, aes(Dissimilarity, max_to_min, colour = Splits)) +
  geom_point(size = 1, alpha = 0.3) + ylim(1.00, fukushima_orig_weight$max_to_min) + ggplot2::ggtitle("Fukushima Dissimilarity vs Max-Min")
ggExtra::ggMarginal(fukushima_marginal, groupColour = TRUE, groupFill = TRUE)
...
ggsave(filename = paste("plots/",
                        ...
                        "_dissim_maxmin.png",
                        sep = ""),
       plot = ggExtra::ggMarginal(fukushima_marginal, groupColour = TRUE, groupFill = TRUE))
...
pref_map_0 <- fukushima_7_map_0

#get plans that have a low max:min ratio (Top 10%)
n <- c(1:nsims)
n <- as.data.frame(n)
pref_smc_weight_0 <- cbind(n, fukushima_7_smc_weight_0)
...
sim_smc_pref_0_good <- fukushima_7_sim_smc_0 %>%
...
pref_0 <- fukushima_7_0
prefadj_0 <- fukushima_7_adj_0
...
pref_map_0 %>%
  ...
  ggnetwork::geom_edges(data = edges_0_adj, mapping = aes(x, y, xend = xend, yend = yend, color = Freq),
             size = 0.8) +
