...
#select municipalities whose population is less than 0% of target pop -> keep intact
pref_small <- pref_0 %>%
  dplyr::filter(pop < 0.0 * sum(pref_0$pop)/ndists_new)
pref_small$subcode <- "0000"
#select municipalities whose population is more than 0% of target pop -> separate
large_codes <- pref_0$code[which(pref_0$code %in% pref_small$code == FALSE)]
pref_large <- pref %>%
  dplyr::filter(code %in% large_codes)
#bind together
pref_0 <- dplyr::bind_rows(pref_small, pref_large)

############Simulation Prep########################
#adjacency list
prefadj <- redist::redist.adjacency(pref_0)

neighbor <- geomander::suggest_neighbors(shp = pref_0,
                                         adjacency = prefadj)
if(nrow(neighbor) > 0){
...
pref_map <- redist::redist_map(pref_0,
                               ndists = round(sum(pref_0$pop)/(sum(pref_0$pop)/ndists_new)),
