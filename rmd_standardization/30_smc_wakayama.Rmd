...
subtitle: "Wakayama Prefecture as an Example"
output: 
  html_document:
  toc: true
  toc_float: true 
...
For Wakayama, there is no Koiki-renkei.
```{r}
...
For Wakayama, no guns are split, so we will not allow any gun to be split.
```{r}
pref_0 <- sf::st_as_sf(
  pref %>%
...
prec_clusters_0 = cutree(cl_co_0, 2) #2: number of clusters
```
