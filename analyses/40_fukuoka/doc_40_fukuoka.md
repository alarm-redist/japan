# 2022 Redistricting for the House of Representatives Electoral Districts in Fukuoka Prefecture
...
* When creating the adjacengy graph, we manually add edges between the following Census blocks, as they are connected by roads:
  + 福岡市東区香椎照葉 (Kashiiteriha, Higashi-ku, Fukuoka-shi) and 福岡市東区大字奈多 (Oaza-nata,  Higashi-ku, Fukuoka-shi); and
  + 福岡市東区香椎照葉 (Kashiiteriha, Higashi-ku, Fukuoka-shi) and 福岡市東区香椎浜ふ頭 (Kashiihamafuto, Higashi-ku, Fukuoka-shi).

## Simulation Notes

* We sample 60,000 districting plans across 4 independent runs of the Sequential Monte Carlo (SMC) algorithm.
* We do not allow municipalities to be split except for those that were split in the 2017 or 2022 map: 福岡市南区 (Minami-ku, Fukuoka-shi), 福岡市城南区 (Jonan-ku, Fukuoka-shi), and 福岡市東区 (Higashi-ku, Fukuoka-shi). We allow these municipalities to be split along the boundaries of Census blocks.
* In addition, we do not allow counties (*gun*) to be split except for those that were split in the 2017 map. In the case of Fukuoka, we do not allow any counties (*gun*) to be split.
* Also, for both regions, we set the sequence of alphas, or the amount by which the weights are adjusted at each resampling stage, at 0.5.
