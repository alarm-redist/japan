...
# number of splits
# Sanbu-gun and , 12409, 12410
merge_gun_exception <- c(12403)  # enter `c()` if not applicable
...
sanbu <- pref_guncode %>%
  dplyr::filter(code == 12403|
                  code == 12409|
                  code == 12410) %>%
  dplyr::mutate(gun_code = code[1])

excp_sanbu <- pref_guncode %>%
  dplyr::filter(code != 12403 &
                  code != 12409 &
                  code != 12410)
...
ggplot() +
  geom_sf(data = optimal_boundary, aes(fill = factor(district)), lwd = 0) +
