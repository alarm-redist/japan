...
new_mun_splits_name <- "" 
new_gun_splits_name <- "安芸郡"
new_koiki_splits_name <- "広島広域都市圏、備後圏域、広島中央地域連携中枢都市圏"
split_1_name <- "福山市"
...
ggplot() +
  ...
  geom_sf_text(data = cities, aes(label = names), color = "black", size = 3,
              nudge_x = c(-6000, 0, 10000), # adjust the position of the labels
              nudge_y = c(9000, 7000, 2000), # adjust the position of the labels
...
PAL <- c('#DCAD35', '#364B7F', '#2A4E45', '#7F4E28', '#6D9537',  '#9A9BB9')
...
redist::redist.plot.plans(sim_smc_pref_1_sample, optimal_1, pref_map_1) +
  ...
  geom_sf(data = cities, size = 2, shape = 21, fill = "red") +
  geom_sf_text(data = cities, aes(label = names), size = 3,
              nudge_x = c(0, -13000, 10000), # adjust the position of the labels
              nudge_y = c(8000, 0, 2000), # adjust the position of the labels
              color = c("black", "white", "white"),
              family = "HiraginoSans-W3") +
...
## Reorder Color Palette
PAL <- c('#6D9537', '#9A9BB9', '#7F4E28', '#DCAD35', '#2A4E45', '#7F4E28')
...
* 逐次モンテカルロ法(SMC)による4つの独立したシミュレーションにより、市区町村分割を許容しない案、最大市区町村の分割を許容する案をそれぞれ`r format(nsims * 4, big.mark = ",", scientific = FALSE)`作成
* 以下の案を棄却し有効な案を選定
  + 飛び地が生じる案
