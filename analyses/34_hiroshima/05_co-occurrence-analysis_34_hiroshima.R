###############################################################################
# Co-occurrence analysis for `34_hiroshima`
...
cities <- data.frame(longitude = c(132.45975600, 133.362474, 132.565010),
                     ...
                     names = c("Hiroshima", "Fukuyama", "Kure"))
...
ggplot() +
  ...
  geom_sf_text(data = cities, aes(label = names), size = 3,
               nudge_x = c(0, 0.02, 0.08), # adjust the position of the labels
               nudge_y = c(0.05, 0.05, 0), # adjust the position of the labels
