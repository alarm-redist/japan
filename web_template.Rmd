...
table_summary <- dplyr::tribble(
  ...
  "改善率", NA, 1-results_0$max_to_min[optimal_0]/sq_max_to_min, 1-results_1$max_to_min[optimal_1]/sq_max_to_min,
...
table_summary %>% 
  ...
  tab_header(
    title = md(paste("**", pref_name_kanji, "**衆議院小選挙区", sep = "")),
...
# 共起分析
* 市区町村分割を許容しない設定で得た区割り案の中で、飛び地がないもののうち、一票の格差が下位10%の案を抽出
...
# アルゴリズム設定・最適解の導出方法
* 各選挙区の人口: (県内の総人口/選挙区数)*±0.10
* 導出する区割り案の数： 25000
* 飛び地が生じた案を棄却した結果得られる有効な区割り案の数:
    + 市区町村を分割しない案: `r length(functioning_results_0$index)`
    + 人口が最大の市区町村を分割する案:`r length(functioning_results_1$index) `
* 有効な区割り案の内一票の格差が最小の案を抽出

### 県特有の設定
飛び地が生じる案を防ぐため、以下の案は棄却
