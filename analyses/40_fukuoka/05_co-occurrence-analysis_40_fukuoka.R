###############################################################################
# Co-occurrence analysis for `40_fukuoka`
...
abline(h = 2.5, col = "red") # explore different depths
abline(h = 2, col = "blue")

# Set the number of clusters (change k to an appropriate number)
k <- 18 # Default: ndists_new
...
cities <- data.frame(longitude = c(130.875050, 130.40171, 130.50841),
                     latitude = c(33.883329, 33.59014, 33.31924),
                     names = c("Kitakyushu", "Fukuoka", "Kurume"))
...
PAL <- c('#6D9537', '#364B7F', '#DCAD35', '#7F4E28', '#9A9BB9', '#2A4E45')
# Co-occurrence plot
ggplot() +
  ...
  geom_sf_text(data = cities, aes(label = names), size = 3,
               # Kita-kyushu, Fukuoka, Kurume
               color = c("black", "black", "black"),
               nudge_x = c(0.16, 0.06, -0.13), # adjust the position of the labels
               nudge_y = c(0, 0.04, 0.02), # adjust the position of the labels
...
# TODO
# Remove additional objects (for Fukuoka only)
rm(sim_smc_pref_no_multi,
   pref_smc_plans_no_multi,
   results_no_multi)
