...
```{r additional packages, include = FALSE, warning = FALSE, message = FALSE}
library(extrafontdb)
library(Rttf2pt1)
```
...
```{r renumber districts (status quo), include = FALSE}
if(length(sq_pref$ku > 6)){
  for(i in sq_pref$ku){
    if(sq_pref$ku[i] %% 6 == 1){
      sq_pref$ku[i] = 1
    } else if(sq_pref$ku[i] %% 6 == 2){
      sq_pref$ku[i] = 2
    } else if(sq_pref$ku[i] %% 6 == 3){
      sq_pref$ku[i] = 3
    } else if(sq_pref$ku[i] %% 6 == 4){
      sq_pref$ku[i] = 4
    } else if(sq_pref$ku[i] %% 6 == 5){
      sq_pref$ku[i] = 5
    } else if(sq_pref$ku[i] %% 6 == 0){
      sq_pref$ku[i] = 6
    }
  }
}
