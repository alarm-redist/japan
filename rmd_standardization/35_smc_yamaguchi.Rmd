...
knitr::opts_knit$set(root.dir = '/home/sho_miyazaki/redist_japan')
```
...
yamaguchi_keno_koiki <- c(35203, 35202, 35204, 35206, 35213, 35216)
hiroshima_koiki <- c(35208, 35212,35305, 35321, 35341, 35343, 35344)
hagi_abu_koiki <- c(35204, 35502)
```

## 2. Run Simulations with 0 Municipality Splits (without taking into accoun "Koiki-renkei")
### 2.1 Finalize pref object
We merge together the gun and treat them as one geographical unit. In Yamaguchi Prefecture, all the gun are kept together in the same district, so we will merge together all the gun.
```{r}
pref_0 <- sf::st_as_sf(
  pref %>%
...
prefadj_0 <- geomander::add_edge(adjacency = prefadj_0,
                                   v1 = 10,
                                   v2 = 14)
...
prefadj_1 <- geomander::add_edge(prefadj_1,
                               ...
                               zero = TRUE)
# add bridge between Oshima-Gun and Yanai City
prefadj_1 <- geomander::add_edge(adjacency = prefadj_1,
                                   v1 = 10,
                                   v2 = 14)
```
...
prefadj_2 <- geomander::add_edge(prefadj_2,
                               ...
                               zero = TRUE)
# add bridge between Oshima-Gun and Yanai City
prefadj_2 <- geomander::add_edge(adjacency = prefadj_2,
                                   v1 = 15,
                                   v2 = 19)
```
...
koiki_1_2[koiki_1_2 %in% c(koiki_1_codes, setdiff(old_2, 35203))] <- 1
# Yamaguchi-shi(35203) is now broken up into 6 units
koiki_2_2 <- pref_2$gun_code
koiki_2_2[koiki_2_2 %in% c(koiki_2_codes)] <- 2
koiki_3_2 <- pref_2$gun_code
koiki_3_2[koiki_3_2 %in% c(koiki_3_codes)] <- 3
```
...
```{r}
optimal_1 <- results_1$index[which(results_1$max_to_min ==
                                         min(results_1$max_to_min))][1]
results_1[optimal_1,]
```
```{r}
optimal_2 <- results_2$index[which(results_2$max_to_min ==
                                         min(results_2$max_to_min))][1]
results_2[optimal_2,]
```
...
koiki_boundary_1 <- pref %>%
  filter(code %in% yamaguchi_keno_koiki) %>%
  summarise(geometry = sf::st_union(geometry))
...
ggplot() +
  ...
  scale_fill_manual(values = c("1" = "green", "2" = "blue", "3" = "orange")) +
  geom_sf(data = mun_boundary, fill = NA, color = "black", lwd = 0.4) +
  geom_sf(data = gun_boundary, fill = NA, color = "black", lwd = 1.0) +
  theme(axis.line = element_blank(), axis.text = element_blank(),
        axis.ticks = element_blank(), axis.title = element_blank(),
