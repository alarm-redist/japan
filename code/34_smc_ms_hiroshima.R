...
optimal_plan <- hiroshima_34_smc_plans_0[, which(hiroshima_34_smc_weight_0$max_to_min == min(hiroshima_34_smc_weight_0$max_to_min))]

split_codes <- unique(pop_by_old_boundary %>% dplyr::filter(X.1 == 9) %>% dplyr::select(X))$X

nsplit <- length(unique(pop_by_old_boundary %>% dplyr::filter(X.1 == 9) %>% dplyr::select(X))$X)
...
for(i in c(nsplit)){
  ...
  for(i in 1:length(pref_n)) {
    init_plan_vec[i] <- optimal_plan[]
  }
