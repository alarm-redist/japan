...
## Color Palette
PAL <- c('#6D9537', '#7F4E28', '#2A4E45', '#DCAD35', '#364B7F','#9A9BB9')
...
ggplot() +
  ...
  geom_sf_text(data = cities, aes(label = names), color = "black", size = 3,
              nudge_x = -0.10, # adjust the position of the labels
...
PAL <- c('#6D9537', '#364B7F', '#DCAD35', '#7F4E28', '#2A4E45')
...
redist::redist.plot.plans(sim_smc_pref_0_sample, optimal_0, pref_map_0) +
  ...
  geom_sf(data = cities, size = 2, shape = 21, fill = "red") +
  geom_sf_text(data = cities, aes(label = names), color = "black", size = 3,
              nudge_x = -9990, # adjust the position of the labels
              nudge_y = 0, # adjust the position of the labels
              family = "HiraginoSans-W3") +
...
# Reorder the color palette
PAL <- c('#DCAD35', '#7F4E28', '#6D9537', '#2A4E45', '#364B7F')
...
* 宮城野区港５丁目は厳密には飛び地とみなすこともできるが、現行の区割りでは飛び地として扱われていないため、本プロジェクトでも飛び地として扱わない。
