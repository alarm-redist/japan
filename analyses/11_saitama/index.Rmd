...
optimal_splits_name <- "さいたま市岩槻区、さいたま市北区、川越市、川口市、富士見市、鴻巣市、熊谷市"
...
table_summary <- dplyr::tribble(
  ...
  "市区町村の分割", sq_mun_splits, results_sample[which(results_sample$index == optimal),]$mun_split - 1, #gun_split is counted in the Urban template 
...
ggplot() +
  ...
  geom_sf_text(data = cities, aes(label = names), size = 3,
              ...
              nudge_x = c(2000, 0, 0, 1000), # adjust the position of the labels
              nudge_y = c(4000, -3000, -5000, -2500), # adjust the position of the labels
              #"さいたま市", "川越市", "川口市", "越谷市"
...
ggplot() +
   ...
   geom_sf(data = cities, size = 2, shape = 21, fill = "red") +
   geom_sf_text(data = cities, aes(label = names), size = 3,
              color = c("white", "black", "black", "black"),
              nudge_x = c(-1500, 0, 0, 0), # adjust the position of the labels
              nudge_y = c(-5000, -4000, -5000, -2000), # adjust the position of the labels
              #"さいたま市", "川越市", "川口市", "越谷市"
              family = "HiraginoSans-W3") +
...
* 隣接する地域と同じ選挙区に属する傾向が強い地域同士の集まりを、k = 22 設定
* 読み取り方  
地図上の地域の色が
  + 同じ → 同じ選挙区に属する傾向が強い
...
ggplot() +
  ...
  geom_sf_text(data = cities, aes(label = names), size = 3,
              ...
              nudge_x = c(0, 0, 0, 0), # adjust the position of the labels
              nudge_y = c(3000, -3000, -5000, -2000), # adjust the position of the labels
              #"さいたま市", "川越市", "川口市", "越谷市"
              family = "HiraginoSans-W3") +
  ggthemes::theme_map(base_family = "HiraginoSans-W3") +
  theme(legend.position = "right", legend.title = element_blank())
...
* 逐次モンテカルロ法(SMC)による2つの独立したシミュレーションにより、区割り改定案を`r format(2 * nsims, big.mark = ",")`作成
