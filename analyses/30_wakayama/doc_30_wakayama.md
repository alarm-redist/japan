# 2022 Redistricting for the House of Representatives Electoral Districts in Wakayama Prefecture
...
* We add edges between 東牟婁郡北山村 (Kitayama-mura, Higashimuro-gun) and 新宮市 (Shingu-shi), as  北山村 is geographically disconnected from the rest of Wakayama Prefecture but is connected to 新宮市 via a road.

## Simulation Notes

* We sample 20,000 districting plans across 4 independent runs of the Sequential Monte Carlo (SMC) algorithm.
* We do not allow municipalities to be split except for those that were split in the 2017 or 2022 map. In the case of Wakayama, we do not allow any municipality to be split.
* In addition, we do not allow counties (*gun*) to be split except for those that were split in the 2017 map. In the case of Wakayama, we do not allow any counties (*gun*) to be split.
