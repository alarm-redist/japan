...
cities <- data.frame(longitude = c(129.877503, 129.715321, 129.28734813700214, 128.84093171202392), 
                     latitude = c(32.750035, 33.179578, 34.2026154097076, 32.69547916489694),
                     names = c("Nagasaki", "Sasebo", "Tsushima", "Goto"))
cities <- sf::st_as_sf(cities, coords = c("longitude", "latitude"), 
                       crs = 4612)

## Information about splits
new_max_to_min <- 1.173
new_mun_splits <- 0
new_gun_splits <- 0
new_koiki_splits <- 0
sq_mun_splits_name <- "長崎市、佐世保市" 
sq_gun_splits_name <- "北松浦郡"
sq_koiki_splits_name <- "長崎広域連携中枢都市圏、西九州させぼ広域都市圏圏"
new_mun_splits_name <- "" 
new_gun_splits_name <- ""
...
boundary_1 <- boundary_1 %>% 
  mutate(type = case_when(
    type == "合併前の市町村の境界" ~ "Historical boundary",
    type == "市区町村の境界" ~ "Municipality boundary",
    type == "郡の境界" ~ "County boundary",
  ))
```
...
# 現行の区割り 
```{r status quo plan, warning=FALSE, message = FALSE, eval=FALSE}
# Set Colors for Plot
if(ndists_old > 6){
  sq_pref_map <- sq_pref %>% 
...
ggplot() +
  ...
  geom_sf_text(data = cities, aes(label = names), color = "black", size = 3,
              nudge_x = c(-0.3, 0.35, 0.3, 0.2), # adjust the position of the labels
              nudge_y = c(0, 0.05, 0, 0), # adjust the position of the labels
...
redist::redist.plot.plans(sim_smc_pref_0_sample, optimal_0, pref_map_0) +
  ...
  geom_sf(data = cities, size = 2, shape = 21, fill = "red") +
  geom_sf_text(data = cities, aes(label = names), color = "black", size = 3,
              nudge_x = c(-35000, 40000, 40000, 30000), # adjust the position of the labels
              nudge_y = c(-5000, 8800, 0, 0), # adjust the position of the labels
              family = "HiraginoSans-W3") +
...
```{r eval=FALSE, "optimal plan (1split)"}
# Reorder the color palette
PAL <- c('#364B7F', '#DCAD35','#6D9537')
...
redist::redist.plot.plans(sim_smc_pref_1_sample, optimal_1, pref_map_1) +
  ...
  geom_sf(data = cities, size = 2, shape = 21, fill = "red") +
  geom_sf_text(data = cities, aes(label = names), color = "black", size = 3,
              nudge_x = c(-32000, 36000, 40000, 30000), # adjust the position of the labels
              nudge_y = c(-5000, 8800, 0, 0), # adjust the position of the labels
              family = "HiraginoSans-W3") +
...
* 現行の区割りでは、空路を介した連続性に配慮し、対馬市・壱岐市は大村市と同一の選挙区とされている。よって、本プロジェクトにおいても、対馬市・壱岐市は大村市と隣接しているものとして扱った。
* 現行の区割りでは、橋で結ばれている西海市と佐世保市は同一の選挙区にされている。よって、本プロジェクトにおいても、西海市は佐世保市と隣接しているものとして扱った。
