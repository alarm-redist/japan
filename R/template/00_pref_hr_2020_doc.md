# 2020 A県　衆議院議員選挙　小選挙区

## 区割り改定時の条件
衆議院議員選挙区画定審議会が策定した [「区割り改定案の作成方針」](https://www.soumu.go.jp/main_content/000794997.pdf)では、以下のような条件が定められている。

* 2020年国勢調査に基づく各選挙区の日本人人口（総人口-外国人人口）は、鳥取２区の日本人人口の２倍未満とする
* 市区町村は原則として分割しない
* 郡は可能な限り分割しない
* 自然的社会的条件を考慮
* 飛び地をもつ選挙区は設けない

なお、「区割り改定案の作成方針」では明文化されていないものの、現行の区割りにおいて市区町村が３分割されている例は極めて稀であること、及び、平成27年の区割り改定時東京都知事が市区町村の３分割を回避するように求め、それが[採択された](https://www.soumu.go.jp/main_content/000761504.pdf)ことから、市区町村を3つ以上の選挙区に分割することは回避すべきとされていると解釈した。
...
## 設定
* 各選挙区の人口 : (県内の総人口 / 選挙区数) ± X %
  * サンプリング効率が90%~100%となるように調節
* 市区町村分割を一切許容しない案、及び日本人人口が最大の市（政令指定都市の行政区を除く）の分割を許容する案の2種類を作成
  * 最大市区町村を分割する際は、平成の大合併以前の市区町村界に基づき分割
  * 旧市区町村単位の（総人口-外国人人口）の値は、該当する市区町村の（総人口-外国人人口）及び旧市区町村単位の日本人人口を元に推計
* 現行の区割りにて分割されている郡のみ、分割を許容
* 離島は、航路で結ばれている本土の市区町村と同一の選挙区にする
* 市区町村分割を一切許容しない案、最大市区町村の分割を許容する案をそれぞれY作成 
* 以下の案を棄却し有効な案を選定
  * 飛び地が生じる案は棄却
  * 市区町村が３つ以上の選挙区に分割される案は棄却
* 有効な案の内、5,000の案を無作為に抽出
...
<!--- [When largest municipality was not split] 日本人人口が最大の市区町村であるいわき市は、1966年以降は市区町村合併を経ておらず、「平成の大合併以前の市区町村界」に該当する市区町村界が存在しない。よって、いわき市の分割を許容する案は作成せず、市区町村分割を一切許容しない案のみを作成した。-->
...
* [「令和2年国勢調査結果(小地域集計　人口等基本集計に関する集計)」(総務省統計局)](https://www.e-stat.go.jp/stat-search/files?page=1&toukei=00200521&tstat=000001136464&cycle=0&tclass1=000001136472)
