###############################################################################
# Data visualization for `Nagasaki`
...
koiki_1_codes <- c(42201, 42300)
koiki_2_codes <- c(42202, 42207, 42208, 42212, 42320, 42380, 42390,
                   42400)
...
koiki_1_1[koiki_1_1 %in% c(koiki_1_codes,
                           setdiff(pref_1$gun_code[which(pref_1$code == split_code)], split_code))] <- 1
...
# No need for filtering in Nagasaki, as there are no marked discontinuities
functioning_results_0 <- results_0
functioning_results_1 <- results_1[which(results_1$multi == 0), ]
