###############################################################################
# Co-occurrence analysis for `22_shizuoka`
...
cities <- data.frame(longitude = c(137.726063, 138.383691),
                     latitude = c(34.710508, 34.975360),
                     names = c("Hamamatsu", "Shizuoka"))
...
ggplot() +
  ...
  scale_discrete_manual("linewidth", values = c(0.2, 0.3, 0.6)) +

  geom_sf(data = cities, size = 2, shape = 21, fill = "red") +
  geom_sf_text(data = cities, aes(label = names), size = 3,
               nudge_x = c(0.03, -0.05), # adjust the position of the labels
               nudge_y = c(0.03, 0.04), # adjust the position of the labels
               color = c("black", "black"),
