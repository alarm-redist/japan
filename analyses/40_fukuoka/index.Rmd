...
cities <- data.frame(longitude = c(130.875050, 130.40171, 130.50841), 
                     ...
                     names = c("北九州市", "福岡市", "久留米市"))
cities <- sf::st_as_sf(cities, coords = c("longitude", "latitude"), 
                       crs = 4612)
## Information about splits
new_max_to_min <- 1.836
new_mun_splits <- 3
new_gun_splits <- 0
new_koiki_splits <- 1
sq_mun_splits_name <- "福岡市南区、福岡市城南区" 
sq_gun_splits_name <- ""
sq_koiki_splits_name <- "北九州都市圏域"
new_mun_splits_name <- "福岡市南区、福岡市城南区、福岡市東区" 
new_gun_splits_name <- ""
new_koiki_splits_name <- "北九州都市圏域"
optimal_splits_name <- "福岡市南区"
optimal_koiki_splits_name <- "久留米広域連携中枢都市圏、北九州都市圏域、嘉飯圏域定住自立圏"
## Color Palette
...
ggplot() +
  ...
  geom_sf_text(data = cities, aes(label = names), size = 3,
              ...
              nudge_x = c(0, 2000, 5000), # adjust the position of the labels
              nudge_y = c(6000, 5000, -6000), # adjust the position of the labels
              # "北九州市", "福岡市", "久留米市"
...
ggplot() +
  ...
  geom_sf_text(data = cities, aes(label = names), size = 3,
              color = c("black", "black", "white"),
              nudge_x = c(-2000, 2000, 5000), # adjust the position of the labels
              nudge_y = c(6000, 5000, -4000), # adjust the position of the labels
              # "北九州市", "福岡市", "久留米市"
              family = "HiraginoSans-W3") +
...
* 隣接する地域と同じ選挙区に属する傾向が強い地域同士の集まりをk = 19設定
* 読み取り方  
地図上の地域の色が
  + 同じ → 同じ選挙区に属する傾向が強い
...
ggplot() +
  ...
  geom_sf_text(data = cities, aes(label = names), size = 3,
              ...
              nudge_x = c(4500, 2000, 5000), # adjust the position of the labels
              nudge_y = c(6000, 5000, -5000), # adjust the position of the labels
              # "北九州市", "福岡市", "久留米市"
              family = "HiraginoSans-W3") +
  ggthemes::theme_map(base_family = "HiraginoSans-W3") +
  theme(legend.position = "right", legend.title = element_blank())
