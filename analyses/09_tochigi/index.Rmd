...
new_max_to_min <- 1.785
new_mun_splits <- 1
new_gun_splits <- 0
new_koiki_splits <- 0
sq_mun_splits_name <- "宇都宮市、栃木市、下野市" 
sq_gun_splits_name <- ""
sq_koiki_splits_name <- "小山地区定住自立圏"
new_mun_splits_name <- "宇都宮市" 
new_gun_splits_name <- ""
new_koiki_splits_name <- ""
split_1_name <- "宇都宮市"

## Color Palette
PAL <- c('#2A4E45', '#DCAD35', '#6D9537', '#7F4E28', '#364B7F',  '#9A9BB9')
```
...
ggplot() +
  ...
  geom_sf_text(data = cities, aes(label = names), color = c("white"), size = 3,
              nudge_x = c(0), # adjust the position of the labels
...
```{r Utsunomiya, include= FALSE}
utsunomiya <- pref %>%
  filter(code == 09201)
utsunomiya_old <- pref_1_with_utsunomiya %>%
  filter(pre_gappei_code == 09201)
```

```{r optimal plan (0 split)}
# Reorder the color palette 
PAL <- c( '#DCAD35', '#7F4E28', '#364B7F', '#6D9537',  '#9A9BB9', '#2A4E45')
redist::redist.plot.plans(sim_smc_pref_0_sample, optimal_0, pref_map_0) +
  ...
  # add Utsunomiya-shi
  geom_sf(data = utsunomiya, fill = '#2A4E45', color = "grey30", lwd = 0.4) +
  
  geom_sf(data = cities, size = 2, shape = 21, fill = "red") +
  geom_sf_text(data = cities, aes(label = names), color = "white", size = 3,
              nudge_x = c(1500), # adjust the position of the labels
...
# Reorder the color palette 
PAL <- c('#6D9537', '#DCAD35', '#364B7F', '#7F4E28', '#9A9BB9', '#2A4E45')
redist::redist.plot.plans(sim_smc_pref_1_sample, optimal_1, pref_map_1) +
  labs(title = element_blank())+
  scale_fill_manual(values = PAL, guide = "none") + 
  # add Utsunomiya-shi
  geom_sf(data = utsunomiya_old, fill = '#2A4E45', color = "grey30", lwd = 0.4) +
...
# Reorder the color palette
PAL <- c('#364B7F', '#6D9537', '#DCAD35', '#7F4E28', '#9A9BB9', '#2A4E45')
...
* 逐次モンテカルロ法(SMC)による4つの独立したシミュレーションにより、市区町村分割を許容しない案、最大市区町村の分割を許容する案をそれぞれ10,000作成
* 以下の案を棄却し有効な案を選定
  + 飛び地が生じる案
...
### 県特有の設定  
* 栃木県足利市羽刈町は他県に囲われた飛び地だが、現行の区割りに従い、本プロジェクトも足利市と地続きとして扱う。
* 栃木県栃木市藤岡町下宮は他県に囲われた飛び地だが、現行の区割りに従い、本プロジェクトも栃木市と地続きとして扱う。
* 栃木県佐野市仙波町は厳密には飛び地とみなすこともできるが、現行の区割りでは飛び地として扱われていないため、本プロジェクトでも飛び地として扱わない。
* 宇都宮市（現行および平成の大合併前の旧宇都宮市）の人口は、県の1議席あたりの人口（県の総人口 / 選挙区数）より大きいため、区割りシミュレーションの対象外とし、宇都宮市・旧宇都宮市単独で選挙区を構成させた。
