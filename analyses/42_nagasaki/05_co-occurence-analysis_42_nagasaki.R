###############################################################################
# Co-occurrence analysis for `42_nagasaki`
...
cities <- data.frame(longitude = c(129.877503, 129.715321),
...
ggplot() +
  ...
  geom_sf_text(data = cities, aes(label = names), size = 3,
               nudge_x = c(-0.28, 0.26), # adjust the position of the labels
               nudge_y = c(0, 0.025), # adjust the position of the labels
               color = c("black", "black"),
