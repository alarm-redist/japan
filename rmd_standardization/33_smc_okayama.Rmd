...
okayama_koiki <- c(33101, 33102, 33103, 33104, 33203, 33204, 33208, 33211, 33212, 
                   33213, 33214, 33346, 33423, 33663, 33666, 33681)
takahashigawa_koiki <- c(33202, 33205, 33207, 33208, 33209, 33210, 33216, 33423,
                         33445, 33461)
bingo_koiki <- c(33205, 33207)
tsuyama_koiki <- c(33203, 33606, 33622, 33623, 33663, 33666)
```

## 2. Run Simulations with 0 Municipality Splits (without taking into accoun "Koiki-renkei")
### For Okayama only: Set aside Kurashiki-shi
```{r}
pref$code[which(pref$pop > sum(pref$pop)/ndists_new)]
```

The population of Kurashiki-shi (33202) is larger than the target population. Thus, for the simulation without any municipality splits, we set aside Kurashiki-shi and treat it as one district. Otherwise, redist_smc is unable to conduct redistricting. We will subsequently run simulations that allows split Kurashiki-shi to be split based on its boundaries before 平成の大合併.

### 2.1 Finalize pref object
We merge together the gun and treat them as one geographical unit. In Okayama Prefecture, all the gun are kept together in the same district, so we will merge together all the gun. Also, we set aside Kurashiki-shi, the municipality whose population is larger than the target population.
```{r}
pref_0 <- sf::st_as_sf(
  pref %>%
    #set aside Kurashiki-shi
...
There are no ferry-related adjacencies to add in Okayama prefecture.

### 2.4 Define redist.map object
```{r}
pref_map_0 <- redist::redist_map(pref_0,
                                 ndists = ndists_new - 1, 
                                 #Kurashiki-shi is treated as one municipality.
                                 pop_tol= 0.15,
                                 total_pop = pop,
                                 adj = prefadj_0)
```
...
We merge together the gun and treat them as one geographical unit, except for the guns that are split under the status quo. We merge together the gun and treat them as one geographical unit. In Okayama Prefecture, all the gun are kept together in the same district, so we will merge together all the gun.
```{r}
pref_1 <- sf::st_as_sf(
  pref_1 %>%
    dplyr::group_by(gun_code) %>%
...
### For Okayama only: Bind together Kurashiki-shi
Since we set aside Kurashiki-shi, we need to consider Kurashiki-shi as one district.
```{r}
...
koiki_4_codes <- c(33203, 33600, 33620, 33660)
...
#count all municipalities except for Kurashiki-shi since it was set aside
koiki_2_0[koiki_2_0 %in% koiki_2_codes[koiki_2_codes %in% 33202 == FALSE]] <- 2
...
koiki_2_1[koiki_2_1 %in% c(koiki_2_codes, setdiff(old_1, 33202))] <- 2
#Kurashiki-shi(33202) is now broken up into 5 units
...
```{r}
optimal_1 <- results_0$index[which(results_1$max_to_min == min(results_1$max_to_min))][1]
results_1[optimal_1,]
```
...
koiki_boundary_1 <- pref %>%
  filter(code %in% okayama_koiki) %>%
  mutate(code = okayama_koiki[1]) %>%
  group_by(code) %>%
  summarise(geometry = sf::st_union(geometry))

koiki_boundary_2 <- pref %>%
  filter(code %in% takahashigawa_koiki) %>%
  mutate(code = takahashigawa_koiki[1]) %>%
  group_by(code) %>%
  summarise(geometry = sf::st_union(geometry))

koiki_boundary_3 <- pref %>%
  filter(code %in% bingo_koiki) %>%
  mutate(code = bingo_koiki[1]) %>%
  group_by(code) %>%
  summarise(geometry = sf::st_union(geometry))

koiki_boundary_4 <- pref %>%
  filter(code %in% tsuyama_koiki) %>%
  mutate(code = tsuyama_koiki[1]) %>%
  group_by(code) %>%
  summarise(geometry = sf::st_union(geometry))
...
ggplot() +
  ...
  scale_fill_manual(values = c("1" = "green", "2" = "yellow", "3" = "blue", 
                               "4" = "orange")) +
  geom_sf(data = mun_boundary, fill = NA, color = "black", lwd = 0.4) +
  geom_sf(data = gun_boundary, fill = NA, color = "black", lwd = 1.0) +
  theme(axis.line = element_blank(), axis.text = element_blank(),
...
ggplot() +
  ...
  #add Kurashikishi
  geom_sf(data = kurashiki, fill = "grey") +
