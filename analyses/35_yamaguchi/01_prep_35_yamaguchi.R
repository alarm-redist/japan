###############################################################################
# Download and prepare data for `35_yamaguchi` analysis
...
sim_type <- "smc"
nsims <- 50000 # Set so that the number of valid plans > 25000
pref_code <- 35
pref_name <- "yamaguchi"
lakes_removed <- c()
ndists_new <- 3
...
sq_gun_splits <- 0
sq_koiki_splits <- 1
pop_tol <- 0.30 # Set so that re-sampling efficiencies are > 80% at each stage
