...
ggplot() +
  ...
  # add Matsuyama-shi
  geom_sf(data = matsuyama, fill = '#DCAD35', color = "grey40", lwd = 0.4) +  
  ...
  geom_sf(data = cities, size = 2, shape = 21, fill = "red") +
  geom_sf_text(data = cities, aes(label = names), color = "white", size = 3,
               nudge_x = c(500), # adjust the position of the labels
               nudge_y = c(7500), # adjust the position of the labels
               family = "HiraginoSans-W3") +
...
* 松山市及び旧松山市の人口は、(県内の総人口 / 選挙区数)を超えるため、松山市及び旧松山市は、必然的に一つの選挙区を構成する。
* 松山市及び旧松山市を除外した上でアルゴリズムを用いた。
