...
###For Hiroshima only: Merge together Aki-gun and Aki-ku to avoid splits
Aki-gun and Aki-ku, Hiroshima-shi both are discontiguous geographical units. In order to avoid creating discontiguous districts, we merge together Aki-gun and Aki-ku and treat that area as one unit. 

```{r}
pref_avoid_enclave <- dplyr::bind_rows(
  #Merge together Aki-gun and Aki-ku
  pref %>% 
    dplyr::filter(code %in% c(34107, 34302, 34304, 34307, 34309)) %>%
    dplyr::mutate(code = replace(code, code %in% c(34107, 34302, 34304, 34307, 34309), 34000),
                  gun_code = replace(gun_code, gun_code %in% c(34107, 34300), 34000)) %>%
    dplyr::group_by(code) %>%
  ...
  #Rest of prefecture
  pref %>% 
    dplyr::filter(code %in% c(34107, 34302, 34304, 34307, 34309) == FALSE)
)
```
