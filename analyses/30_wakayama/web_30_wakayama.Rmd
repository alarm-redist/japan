...
table_summary <- dplyr::tribble(
  ...
  "県内較差", sq_max_to_min, results_0_sample[which(results_0_sample$index == optimal_0),]$max_to_min, 
  "改善率", NA, (sq_max_to_min - results_0_sample[which(results_0_sample$index == optimal_0),]$max_to_min)/(sq_max_to_min - 1), 
  "選挙区数", ndists_old, ndists_new,
  "市区町村の分割", sq_mun_splits, 0, 
  "郡の分割", sq_gun_splits, sim_gun_split, 
  "広域連携地域の分割", sq_koiki_splits, results_0_sample[which(results_0_sample$index == optimal_0),]$koiki_split
) 
...
# アルゴリズムの提案
アルゴリズムが提案した5,000案のうち、有効な案の中で、一票の格差が最小のもの
 ...
 * 市区町村分割を許容しない案、最大市区町村の分割を許容する案をそれぞれ`r format(nsims, big.mark = ",")`作成 
 * 以下の案を棄却し有効な案を選定
   + 飛び地が生じる案
