###############################################################################
# Co-occurrence analysis for `23_aichi`
...
cities <- data.frame(longitude = c(136.906695, 137.156734, 137.391677, 137.173516, 136.802146),
                     ...
                     names = c("Nagoya", "Toyota", "Toyohashi", "Okazaki", "Ichinomiya"))
...
PAL <- c('#6D9537', '#DCAD35', '#364B7F', '#7F4E28', '#2A4E45', '#9A9BB9')
# Co-occurrence plot
ggplot() +
  ...
  geom_sf_text(data = cities, aes(label = names), size = 3,
               ...
               nudge_x = c(0.05, 0.01, 0.05, 0.1, -0.05), # adjust the position of the labels
               nudge_y = c(-0.03, 0.03, -0.03, 0, 0.03), # adjust the position of the labels
               # Nagoya, Toyota, Toyohashi, Okazaki, Ichinomiya
...
ggplot() +
  ...
  geom_sf_text(data = cities, aes(label = names), size = 3,
               color = c("black", "black", "black", "black", "black"),
               nudge_x = c(0.05, 0.05, 0.05, 0.1, 0.05), # adjust the position of the labels
               nudge_y = c(0.03, 0.03, -0.03, 0, 0.03), # adjust the position of the labels
               # Nagoya, Toyota, Toyohashi, Okazaki, Ichinomiya
               family = "HiraginoSans-W3") +
...
# TODO
# Remove additional objects (for Aichi only)
rm(pref_shp_2020_fixed,
   hanaoka,
   hisaoka_1,
   mainland_owari,
   mainland_owari_adj,
   ...
   owari_sep,
   owari_smc_plans,
   owari_smc_plans_no_multi,
   results_owari,
   results_owari_no_multi,
   sim_smc_owari,
   sim_smc_owari_no_multi,
   functioning_results_owari,
   wgt_smc_owari
   )
