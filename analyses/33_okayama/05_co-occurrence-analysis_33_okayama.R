###############################################################################
# Co-occurrence analysis for `33_okayama`
...
cities <- data.frame(longitude = c(133.919397, 133.771762),
                     latitude = c(34.655324, 34.585103),
                     names = c("Okayama", "Kurashiki"))
...
ggplot() +
  ...
  geom_sf_text(data = cities, aes(label = names), size = 3,
               nudge_x = c(-0.025, -0.02), # adjust the position of the labels
               nudge_y = c(0.05, -0.02), # adjust the position of the labels
               color = c("black", "black"),
