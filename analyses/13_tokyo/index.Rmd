...
optimal_splits_name <- "港区、江東区、品川区、大田区、世田谷区、渋谷区、杉並区、豊島区、板橋区、練馬区、足立区、江戸川区、八王子市、青梅市、調布市、小平市、多摩市"
...
table_summary <- dplyr::tribble(
  ...
  "市区町村の分割", sq_mun_splits, optimal_splits, 
  "郡の分割", sq_gun_splits, 0, 
  "広域連携地域の分割", sq_koiki_splits,0
) 
table_summary %>% 
...
ggplot() +
  ...
  geom_sf(data = boundary_special_wards, aes(color = type), show.legend = "line", 
          fill = NA, lwd = 0.4) +
...
redist::redist.plot.plans(sim_smc_tama_sample, optimal_tama, tama_map) +
  ...
  geom_sf(data = boundary_tama, aes(color = type), show.legend = "line", 
          fill = NA, lwd = 0.4) +
...
* 区割り改定案を23区地域、多摩地域それぞれ`r format(nsims, big.mark = ",")`作成 
