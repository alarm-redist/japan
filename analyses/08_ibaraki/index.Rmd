...
new_max_to_min <- 1.910
new_mun_splits <- 0
new_gun_splits <- 1
new_koiki_splits <- 1
sq_mun_splits_name <- "水戸市、下妻市、笠間市、 常陸大宮市、小美玉市"
sq_gun_splits_name <- "東茨城郡"
sq_koiki_splits_name <- "茨城県央地域定住自立圏"
new_mun_splits_name <- "" 
new_gun_splits_name <- "東茨城郡"
new_koiki_splits_name <- "茨城県央地域定住自立圏"
split_1_name <- "水戸市"
...
ggplot() +
  ...
  geom_sf_text(data = cities, aes(label = names), color = "black", size = 3,
              ...
              nudge_y = c(-4000), # adjust the position of the labels
...
### 県特有の設定  
つくば市寺具は厳密には飛び地とみなすこともできるが、現行の区割りでは飛び地として扱われていないため、本プロジェクトでも飛び地として扱わない。
