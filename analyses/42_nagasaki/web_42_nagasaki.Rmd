...
cities <- data.frame(longitude = c(129.868131, 129.715184), 
                     latitude = c(32.750410, 33.180028),
                     names = c("長崎市", "佐世保市"))
...
sq_koiki_splits_name <- "長崎広域連携中枢都市圏、西九州させぼ広域都市圏"
split_1_name <- "長崎市"

## Color Palette
PAL <- c('#2A4E45', '#7F4E28', '#364B7F', '#9A9BB9')
```
...
ggplot() +
  ...
  geom_sf_text(data = cities, aes(label = names), color = "black", size = 2,
              nudge_x = c(-0.3, 0.35), # adjust the position of the labels
              nudge_y = c(0, 0.04), # adjust the position of the labels
...
ggplot() +
  ...
  geom_sf(data = cities, size = 2, shape = 21, fill = "red") +
  geom_sf_text(data = cities, aes(label = names), color = "black", size = 2,
               nudge_x = c(-30000, 35000), # adjust the position of the labels
               nudge_y = c(0, 6000), # adjust the position of the labels
               family = "HiraginoSans-W3") +
