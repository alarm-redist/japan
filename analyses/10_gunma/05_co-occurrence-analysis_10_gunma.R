###############################################################################
# Co-occurrence analysis for `10_gunma`
...
cities <- data.frame(longitude = c(139.063545, 139.0032835),
                     latitude = c(36.3893955, 36.321787),
                     names = c("Maebashi", "Takasaki"))
...
PAL <- c('#9A9BB9', '#DCAD35', '#6D9537', '#7F4E28', '#2A4E45', '#364B7F')
# Co-occurrence plot
ggplot() +
  ...
  geom_sf_text(data = cities, aes(label = names), size = 3,
               nudge_x = c(0.06, 0.03), # adjust the position of the labels
               nudge_y = c(0.03, -0.02), # adjust the position of the labels
               color = c("black", "black"),
...
ggplot() +
  ...
  geom_sf(data = cities, size = 2, shape = 21, fill = "red") +
  geom_sf_text(data = cities, aes(label = names), size = 3,
               nudge_x = c(0.06, 0), # adjust the position of the labels
               nudge_y = c(0.03, -0.03), # adjust the position of the labels
               color = c("white", "white"),
               family = "HiraginoSans-W3") +
