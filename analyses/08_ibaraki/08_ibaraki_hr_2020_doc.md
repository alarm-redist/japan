...
* 逐次モンテカルロ法(SMC)による4つの独立したシミュレーションにより、市区町村分割を許容しない案、最大市区町村の分割を許容する案をそれぞれ60,000作成 
* 以下の案を棄却し有効な案を選定
  * 飛び地が生じる案
...
## データの処理
* つくば市寺具を飛び地として扱わないため、シュミレーション前の`pref_map`作成時に飛地部分をシェイプファイルから除外した。
