###############################################################################
# Co-occurrence analysis for `13_tokyo`
...
mun_boundary <- pref_shp_cleaned %>%
  filter(code < 13360) %>% # exclude islands
  group_by(code) %>%
...
cities <- data.frame(longitude = c(139.315780),
                     latitude = c(35.666113),
                     names = c("Hachioji"))
...
# Co-occurrence plot
ggplot() +
  geom_sf(data = filter(pref_cooc, code < 13360), # Filter out islands
          aes(fill = as.factor(color), alpha = cooc_ratio), show.legend = FALSE) +
  ...
  geom_sf_text(data = cities, aes(label = names), size = 3,
               ...
               nudge_y = -0.03, # adjust the position of the labels
...
# Plot Map
ggplot() +
  geom_sf(data = filter(color_pref_map,code < 13360), # Filter out islands
          aes(fill = factor(color)), color = NA) +
  scale_fill_manual(values = PAL, guide = "none") +
...
# TODO
# Remove additional objects (for Tokyo only)
rm(constr_special_wards,
   init,
   koto_map,
   setagaya_map,
   special_wards_map,
   ...
   results_no_multi,
   results_special_wards_no_multi,
   wgt_smc_special_wards,
   special_wards_smc_plans,
   special_wards_smc_plans_no_multi,
   special_wards_sep,
   special_wards_largest,
   special_wards_largest_adj,
   special_wards_add_edge,
   functioning_results_special_wards,
   mun_split_special_wards,
   num_mun_split_special_wards,
   no_multi,
   no_multi_special_wards,
   sim_smc_pref_no_multi,
   sim_smc_special_no_multi)
