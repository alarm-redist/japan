...
redist::redist.plot.map(
  pref_part,
  fill = overlap_vec[, which(overlap_hash == overlap_counts$overlap_hash[index])[1]],
  plan = block_div,
  title = paste("Minimum Max-Min Ratio:", round(min(part_parallel_weight_pref[good_num_0[which(overlap_hash == overlap_counts$overlap_hash[index])], ]$max_to_min), digits = 3))
)
...
plot_ms <- ggplot(best_weights, aes(dissimilarity, max_to_min, colour = splits)) +
  geom_point(size = 1, alpha = 0.5) + ggplot2::ggtitle("Kanagawa Dissimilarity vs Max-Min")
...
blocks <- list(unique(pref_block$code)[which(unique(pref_block$code) <= 14120)],
               unique(pref_block$code)[which(unique(pref_block$code) >= 14120 & unique(pref_block$code) <= 14140)],
               unique(pref_block$code)[which(unique(pref_block$code) >= 14150)])
...
kanagawa_blocks <- block_plans_pref[, which(redist::redist.parity(block_plans_pref, pref_block$pop) == min(redist::redist.parity(block_plans_pref, pref_block$pop)))[1]]
...
for (j in 1:n_blocks) {
  ...
  smallest_x <- pref_block %>% dplyr::filter(code %in% freeze)

  pref_part <- dplyr::bind_rows(small_units %>% dplyr::filter(!(code %in% smallest_x$code) & code %in% part_codes & code %ni% unique(pref_block$code)[45:50]), pref_block %>% dplyr::filter(code %in% smallest_x$code & code %in% part_codes
                                                                                                                                                                                            | code %in% unique(pref_block$code)[45:50]))
  ...
  part_plans_pref <- redist::get_plans_matrix(part_parallel_pref)

  # get disparity data
  part_weight_pref <- simulation_weight_disparity_table(part_parallel_pref)
