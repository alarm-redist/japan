...
ggplot() +
  ...
  geom_sf_text(data = cities, aes(label = names), color = "black", size = 3,
              nudge_x = c(0, 0, 0, 0, 0), # adjust the position of the labels
              nudge_y = c(0, 0, 0, 0, 0), # adjust the position of the labels
...
```{r optimal plan}
redist::redist.plot.plans(sim_smc_pref_sample, optimal, pref_map) +
...
* 隣接する地域と同じ選挙区に属する傾向が強い地域同士の集まりをk = X 設定
* 読み取り方  
地図上の地域の色が
  + 同じ → 同じ選挙区に属する傾向が強い
