...
new_max_to_min <- 1.155
new_mun_splits <- 1
new_gun_splits <- 0
new_koiki_splits <- 0
sq_mun_splits_name <- "高崎市、桐生市、太田市、 渋川市、みどり市" 
sq_gun_splits_name <- ""
sq_koiki_splits_name <- ""
new_mun_splits_name <- "高崎市" 
new_gun_splits_name <- ""
new_koiki_splits_name <- ""
split_1_name <- "高崎市"
split_1_koiki_splits_name <- "利根沼田地域定住自立圏"

## Color Palette
PAL <- c('#6D9537', '#364B7F', '#DCAD35', '#9A9BB9', '#7F4E28', '#2A4E45')
```
...
table_summary %>% 
 ...
 tab_footnote(
    footnote = split_1_koiki_splits_name,
    locations = cells_body(
      columns = 5,
      rows = 6
    ) 
...
注: 市区町村を分割しないシミュレーション結果の区割り案の多様性 (`plan diversity`)が、ソフトウェアが設定する理想値より低い。
...
PAL <- c('#6D9537', '#9A9BB9', '#DCAD35', '#364B7F', '#7F4E28', '#2A4E45')
redist::redist.plot.plans(sim_smc_pref_0_sample, optimal_0, pref_map_0) +
  ...
  geom_sf(data = cities, size = 2, shape = 21, fill = "red") +
  geom_sf_text(data = cities, aes(label = names), color = "black", size = 3,
              nudge_x = c(10000, 0), # adjust the position of the labels
              nudge_y = c(3000, -5000), # adjust the position of the labels
              family = "HiraginoSans-W3") +
...
PAL <- c('#6D9537', '#9A9BB9', '#364B7F', '#DCAD35', '#7F4E28', '#2A4E45')
