# 2022 Redistricting for the House of Representatives Electoral Districts in Ehime Prefecture
...
* We enforce a maximum population deviation of 35%.
* We also add a constraint to encourage the creation of geographically compact districts.

## Pre-processing Notes

* We add edges between islands and municipalities to which the islands are connected via ferry routes. In the case of Ehime, we add edges between mainland 松山市 (Matsuyama-shi) and 旧中島町 (what used to be Nakajima-cho) as well as between 今治市 (Imabari-shi) and 上島町 (Kamijima-cho).

## Simulation Notes

* We sample 20,000 districting plans across 4 independent runs of the Sequential Monte Carlo (SMC) algorithm.
* We do not allow municipalities to be split except for those that were split in the 2017 or 2022 map. In the case of Ehime, we allow 松山市 (Matsuyama-shi) to be split along the boundaries of pre-merger municipalities.
* In addition, we do not allow counties (*gun*) to be split except for those that were split in the 2017 map. In the case of Ehime, we do not allow any counties (*gun*) to be split.
