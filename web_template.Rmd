...
# 共起分析
* 市区町村分割のない設定で有効な案の中のうち一票の格差が下位10%の案を抽出
* 同色の市区町村は、同じ選挙区に属する傾向が大きい
* 同色の市区町村のうち色の濃い組み合わせの方が、同じ選挙区に属する傾向が大きい
...
* 導出する区割り案の数： 25000
* 飛び地が生じた案を棄却した結果得られた、有効な区割り案の数:
    + 市区町村を分割しない案: `r length(functioning_results_0$index)`
    + 人口が最大の市区町村の分割を許容する案: `r length(functioning_results_1$index) `
