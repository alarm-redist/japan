...
new_max_to_min <- 1.222
new_mun_splits <- 0
new_gun_splits <- 0
new_koiki_splits <- 2
sq_mun_splits_name <- "岡山市北区・南区・東区、吉備中央町、真庭市、倉敷市" 
sq_gun_splits_name <- "加賀郡"
sq_koiki_splits_name <- "岡山連携中枢都市圏、高梁川流域連携中枢都市圏"
new_mun_splits_name <- "" 
new_gun_splits_name <- ""
new_koiki_splits_name <- "岡山連携中枢都市圏、高梁川流域連携中枢都市圏"
koiki_splits_name <- "岡山連携中枢都市圏、高梁川流域連携中枢都市圏、津山圏域定住自立圏"
split_1_name <- "倉敷市"
```
...
## Color palette
PAL <- c('#9A9BB9', '#DCAD35', '#364B7F', '#7F4E28', '#6D9537')
...
ggplot() +
  ...
  geom_sf_text(data = cities, aes(label = names),  size = 3,
               color = c("black", "white"),
              nudge_x = c(-0.04, 0), # adjust the position of the labels
              nudge_y = c(0.035,-0.035), # adjust the position of the labels
...
redist::redist.plot.plans(sim_smc_pref_0_sample, optimal_0, pref_map_0) +
  ...
  # add Kurashiki-shi
  geom_sf(data = kurashiki, fill = '#7F4E28', color = "grey30", lwd = 0.4) + 
  ...
  geom_sf_text(data = cities, aes(label = names),  size = 3,
              color = c("white", "white"),
              nudge_x = c(-5000, 0), # adjust the position of the labels
              nudge_y = c(5000,-5000), # adjust the position of the labels
              family = "HiraginoSans-W3") +
...
PAL <- c('#DCAD35', '#364B7F', '#7F4E28', '#6D9537')
...
* 倉敷市の人口は、県の1議席あたりの人口（県の総人口 / 選挙区数）より大きい。そのため、最大市区町村の分割を許容しない区割りシミュレーションでは、倉敷市を対象外とし、単独で選挙区を構成させた。
