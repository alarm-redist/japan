###############################################################################
# Co-occurrence analysis for `30_wakayama`
...
cities <- data.frame(longitude = c(135.17086),
                     latitude = c(34.23035),
                     names = c("Wakayama"))
...
PAL <- c('#6D9537', '#DCAD35', '#9A9BB9', '#364B7F', '#2A4E45', '#7F4E28')
# Co-occurrence plot
ggplot() +
  ...
  geom_sf_text(data = cities, aes(label = names), size = 3,
               nudge_x = c(-0.14), # adjust the position of the labels
