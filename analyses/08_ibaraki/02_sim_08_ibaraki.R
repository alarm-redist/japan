...
# To avoid 飛地, we merge 常総市, 下妻市, 坂東市, 古河市, 結城郡, and 猿島郡 together,
# because the borders of those municipalities contain 飛地 which are respected under the status quo.
# Assign 猿島郡's gun_code to merge.
pref <- pref %>%
...
# つくば市(09204) is made up of discontiguous parts (飛地).
# However, in practice, つくば市 is treated as a contiguous unit under the enacted plan.
# Thus, for the purpose of checking whether plans are contiguous or not,
# we remove 飛地 of つくば市(09204) from the shape file.
tsukuba <- pref %>%
  dplyr::filter(code==8220) %>%
  # Convert MULTIPOLYGON to several POLYGONs
  sf::st_cast("POLYGON") %>%
  # Calculate the area, and order by the area size
  dplyr::mutate(area = sf::st_area(geometry)) %>%
  dplyr::arrange(desc(area))

# Assigned the POLYGON of つくば市 without 飛地
pref[which(pref$code==8220),]$geometry <- tsukuba[1,]$geometry
