...
split_code <- as.character(split_code) # There is no splits for Wakayama in the status quo districting
...
# In Wakayama, as the status quo does not split any municipalities, we also do not split municipalities.
# Therefore, the code above does not change the number of rows, and the added column `pre_gappei_code`
# is identical with `code`.

# Replace NA values in `old_mun_name`
pref_mun_old$old_mun_name <- "-"
...
# For Wakayama, it was impossible to increase the plan diversity further.
