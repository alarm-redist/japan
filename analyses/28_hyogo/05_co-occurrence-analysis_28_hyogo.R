###############################################################################
# Co-occurrence analysis for `28_hyogo`
...
cities <- data.frame(longitude = c(135.195794, 134.685579, 135.341088, 135.406389, 134.997116),
                     ...
                     names = c("Kobe", "Himeji", "Nishinomiya", "Amagasaki", "Akashi"))
...
ggplot() +
  ...
  geom_sf_text(data = cities, aes(label = names), size = 3,
               # Kobe, Himeji, Nishinomiya, Amagasaki, Akashi
               color = c("black", "black", "black", "black", "black"),
               nudge_x = c(0.14, 0.06, -0.09, -0.02, 0), # adjust the position of the labels
               nudge_y = c(-0.05, 0.03, 0.06, 0.03, -0.04), # adjust the position of the labels
...
# TODO
# Remove additional objects (for Osaka only)
rm(constr_west,
   init,
   west_map,
   mainland_west,
   mainland_west_adj,
   results_no_multi,
   results_west_no_multi,
   wgt_smc_west,
   west_smc_plans,
   west_smc_plans_no_multi,
   west_sep,
   west_largest,
   west_largest_adj,
