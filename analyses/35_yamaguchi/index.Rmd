...
new_max_to_min <- 466111/413267 
# use exact number to avoid discrepancy between max:min ratio of optimal map and 
# max:min ratio of new map
new_mun_splits <- 0
new_gun_splits <- 0
new_koiki_splits <- 1
sq_mun_splits_name <- "山口市、周南市" 
sq_gun_splits_name <- ""
sq_koiki_splits_name <- "山口県央連携都市圏域"
new_mun_splits_name <- "" 
new_gun_splits_name <- ""
new_koiki_splits_name <- "山口県央連携都市圏域"
split_1_name <- "下関市"
...
table_summary <- dplyr::tribble(
  ...
  "改善率", NA, (sq_max_to_min - new_max_to_min)/(sq_max_to_min - 1),
          (sq_max_to_min - results_0_sample[which(results_0_sample$index == optimal_0),]$max_to_min)/(sq_max_to_min - 1),  # round max:min
...
ggplot() +
  ...
  geom_sf_text(data = cities, aes(label = names), color = c("white", "black"), size = 3,
              nudge_x = c(0.125), # adjust the position of the labels
...
PAL <- c('#DCAD35', '#6D9537', '#364B7F', '#9A9BB9', '#2A4E45', '#7F4E28')
redist::redist.plot.plans(sim_smc_pref_0_sample, optimal_0, pref_map_0) +
  ...
  geom_sf(data = cities, size = 2, shape = 21, fill = "red") +
  geom_sf_text(data = cities, aes(label = names), color = c("white", "black"), size = 3,
               nudge_x = c(12500), # adjust the position of the labels
               nudge_y = c(0), # adjust the position of the labels
               family = "HiraginoSans-W3") +
...
redist::redist.plot.plans(sim_smc_pref_1_sample, optimal_1, pref_map_1) +
  ...
  geom_sf(data = cities, size = 2, shape = 21, fill = "red") +
  geom_sf_text(data = cities, aes(label = names), color = c("white", "black"), size = 3,
               nudge_x = c(10000), # adjust the position of the labels
               nudge_y = c(0), # adjust the position of the labels
               family = "HiraginoSans-W3") +
